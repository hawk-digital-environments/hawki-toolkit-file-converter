...
async def process_plain_text(
    ...
    """Process a plain text file that kreuzberg does not natively support.

    Reads the file with the detected encoding, then feeds it through the
    same chunking + keyword pipeline as kreuzberg-based extraction.

    Args:
        file_path: Path to the uploaded file in the temp directory.
        zip_dir: Output directory (where chunks/ and meta.json go).
        encoding: Text encoding detected by charset-normalizer.
    """
    raw_bytes = file_path.read_bytes()
    try:
        text = raw_bytes.decode(encoding)
    except (UnicodeDecodeError, LookupError) as e:
        raise RuntimeError(f"Failed to decode file as {encoding}: {e}") from e

    languages = [os.getenv("OCR_FALLBACK_LANGUAGE", "de")]
    max_chunk_length = int(os.getenv("MAX_CHUNK_LENGTH", 3000))

    chunk_dir = zip_dir / "chunks"
    chunk_dir.mkdir(parents=True, exist_ok=True)

    async def _text_nodes() -> AsyncGenerator[ElementNode]:
        yield ElementNode(content=text, starts_new_page=False, page_number=None)

    nodes = _text_nodes()
    chunks = accumulate_chunks(nodes, max_chunk_length, has_pages=False)
    ...
    _write_plain_text_metadata(file_path, total_chunks, languages, all_keywords, zip_dir, encoding)


def _write_plain_text_metadata(
    file_path: Path,
    total_chunks: int,
    languages: list[str],
    keywords: list[str],
    ...
    extraction_metadata: dict = {
        ...
        "size": file_path.stat().st_size,
        "chunks": total_chunks,
        "mimeType": f"text/plain; charset={encoding}",
    }
    if languages:
        extraction_metadata["languages"] = languages
    if keywords:
...
async def process_file_core(
    ...
    with TemporaryDirectory() as tmp_base:
        ...
        if is_image_filename(filename):
            ...
            await process_plain_text(file_path, zip_dir, text_encoding)
        else:
...
async def process_image_content(tmp_imagefile_path: Path, assets_dir: Path, language: str = "de"):
