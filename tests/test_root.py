...
def test_root_returns_service_info(
    ...
    assert len(expected_formats.difference(body["supported_formats"])) == 0, (
        f"Unexpected supported format {expected_formats.difference(body['supported_formats'])}"
    )
    assert len(expected_image_formats.difference(body["image_formats"])) == 0, (
    ...
    assert "text_fallback_note" in body
    assert "plain text" in body["text_fallback_note"].lower()
