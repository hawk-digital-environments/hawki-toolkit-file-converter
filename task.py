...
TEMPORAL_MAX_CACHED_WORKFLOWS = int(os.environ.get("TEMPORAL_MAX_CACHED_WORKFLOWS", "1000")) # 1000 is temporal default
...
async def process_file_activity(
    ...
    print(
        f"file_path={file_path} filename={filename} "
        f"result_dir={result_dir} text_encoding={text_encoding}"
    )
    file_bytes = Path(file_path).read_bytes()
    zip_bytes, headers = await process_file_core(file_bytes, filename, text_encoding=text_encoding)
